...
- 零节点冷启动时，Sub-Store 上游订阅获取可使用链接中显式声明的 `proxy=DIRECT`。这是唯一新增的控制面直连例外；它会向订阅提供者暴露真实出口 IP，但不会把 `DIRECT` 放入任何境外业务策略组。
...
主配置把经审核的 22 个规则快照内嵌为 `[Ruleset RS_*]`，共 8115 条有效项。Surge iOS 5.7.0 起支持 Inline Ruleset；本配置最低版本 5.14.6 满足要求。
...
更新源快照时先修改 `Rules/`，运行 `python3 tools/embed_runtime_rules.py Surge.conf` 重新生成主配置，再显式更新审计哈希。不能直接在内嵌段中手改后跳过源文件。
...
```text
PASS: Surge.conf | groups=31 rules=321 inline_rulesets=22 inline_entries=8115 external_rules=0 direct_groups=Apple,Domestic
PASS: Rules | files=32 active_entries=132575 target=Surge-iOS runtime_files=22 runtime_entries=8115
PASS: audit_config regression cases=28
```
