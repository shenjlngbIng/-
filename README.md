...
规则整理。当前版本重点降低节点失效、DNS 故障、IPv6 网络、Apple 系统流量及
...
| Apple、系统服务、APNs | `DIRECT` | 减少登录、下载、系统校验和推送随代理切换中断的概率 |
...
- Apple 系统规则与 APNs 默认直连；APNs 的 `akadns.net` 规则已收窄到 Apple Push
  专用后缀，避免误匹配其他厂商域名。
...
- 推送异常：确认 `Apple Push` 为 `DIRECT`，然后开关飞行模式重建 APNs 长连接。
...
- `include-all-networks=true` 用于降低 App 绑定物理接口绕过 VIF 的可能性，但可能影响
  AirDrop、投屏及部分调试功能。
- Apple 与系统流量默认 DIRECT，优先保证系统可用性；这部分连接会使用真实出口。
