...
| `Fallback` | 每 60 秒检查节点，选择第一个可用项。 | 否 |
| `Proxy` | 全局代理入口，可在自动、故障转移、地区组之间切换。 | 否 |
| `Final` | 未命中规则的最终出口。 | 否 |
...
APNs 域名/IP          → Apple Push → APNs Proxy → DIRECT 兜底
```
...
7. 若代理节点无法连接 APNs，等待 `Apple Push` 自动回落到 `DIRECT` 后再次测试。
...
这是预期行为。`REJECT` 不是代理节点，而是订阅为空或地区筛选无结果时的失败保护。在 `AllServer` 手动选择页中不要主动选择它；`Auto` 和地区 Smart 策略会通过优先级把它放在真实节点之后，`Fallback` 的连通性检查也会跳过不可用的 `REJECT`。若整个 `AllServer` 只有 `REJECT`，说明订阅地址尚未填写、加载失败或节点名称全部被过滤。
...
4. 本主配置不包含脚本、MITM 或 URL Rewrite；若出现 JSON 字段缺失、脚本运行错误或通知轰炸，来源通常是另外启用的模块。
5. 不要同时加载多个功能重叠的大型广告规则集。
