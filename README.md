...
- 本地 `.list` 文件是否可读。
- 主配置引用的 22 个运行时文件是否无重复且全部存在于候选包。
- 规则类型和字段数量是否处于允许范围。
- Surge iOS 会忽略的 `PROCESS-NAME` 是否被重新混入。
- 无 MITM 移动配置不采用的 `URL-REGEX` 是否被重新混入。
- 运行时 IPv4、IPv6、ASN 规则是否统一带有 `no-resolve`。
- DOMAIN-SET 是否含空白或逗号等格式错误。
- 声明的规则统计是否与实际有效项一致。
- 文件内是否存在完全重复项、非法 IP 或超长规则。
...
- 保留 `USER-AGENT`，但仅视为尽力匹配。iOS 15 以后，无 MITM 时大多数 HTTPS 请求的真实 User-Agent 不可见，因此它不能承担代理闭锁或防泄漏职责；未命中仍由域名规则和 `FINAL` 收口。
- 运行时所有 `IP-ASN`、`IP-CIDR`、`IP-CIDR6` 条目必须带 `no-resolve`，避免规则集在域名阶段意外触发本地解析。

完整来源、许可状态、实际分发与仅供参考的项目区分见 `NOTICE.md`。部分历史规则快照仍缺少逐文件来源元数据；在补齐之前，不应把它们宣称为本项目原创，也不应无条件扩大再分发。

### 12.4 对比项目中采用的维护方法

本项目只吸收适合当前威胁模型的工程方法，不直接拼接其他项目的完整配置，也不新增它们的可变运行时 URL：

| 参考项目 | 采用的做法 | 明确未采用的内容 |
| --- | --- | --- |
| SukkaW/Surge | 按域名、非 IP、IP 分层审查；尽量先处理域名/非 IP；按客户端能力过滤规则类型 | Clash/Mihomo 输出、测速规则、MITM 模块、大体量移动端广告集、远程直连 |
| DivineEngine 规则说明与 Profiles | 自上而下匹配、IP 规则使用 `no-resolve`、唯一 `FINAL` 收口 | 旧配置整包、宽泛内置直连规则、未经本威胁模型复核的策略 |
| blackmatrix7/ios_rule_script | 平台专属产物思路；在 NOTICE 记录来源/本地修改，并自动核对已有规则计数 | `master`/`release` 可变运行时链接、脚本、Rewrite、MITM、非 Surge 格式 |
| Rabbit-Spec/Surge | 候选更新前后差异复核、保留本地排除层的维护思路 | 模块、脚本、MITM、外部控制面及整份个人配置 |
| GeQ1an/Rules、NobyDa/ND-AD | 仅保留来源追溯和历史对照价值 | 不作为新规则来源或运行时依赖 |

这样做的重点是让“上游内容”先变成可审查候选，再经过 iOS 能力过滤、审计器固化的排除断言、静态检查和固定提交发布；任何来源都不能仅凭知名度获得直连权限。

### 12.5 更新远程规则的安全流程

1. 在隔离目录下载候选文件。
2. 记录原始项目、文件 URL、提交、许可证和获取日期。
3. 对比新增/删除规则，特别检查策略字段、逻辑规则、过宽网段、异常长行和来源突然变化。
4. 应用 `audit_rules.py` 固化的 iOS 排除规则，并过滤 `PROCESS-NAME`、`URL-REGEX` 及其他非 Surge iOS 能力项。
5. 运行 `python3 tools/audit_rules.py Rules --profile Surge.conf`。
6. 先提交通过审核的规则快照，取得新的 40 位完整提交号。
7. 再将 `Surge.conf`、`tools/audit_config.py` 中的固定提交同步替换为该提交号；不得临时改成 `main`、`master` 或 `release`。
8. 重新运行全部审计，在真机验证缓存存在、缓存清除、代理全断三种状态。
9. 更新 `README.md`、`NOTICE.md` 的统计、来源和本地修改说明。
