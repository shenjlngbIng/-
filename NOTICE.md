...
本项目在公开配置基础上进行了大幅重组与安全加固，包括策略闭环、规则顺序、DNS/DoH、IPv4/IPv6、UDP/QUIC/STUN、APNs 独立回退、控制面收缩、远程规则固定提交、只读候选包验证及静态检查。
...
- 只有经审核并内联的国内、Apple、APNs 和局域网目标可以到达直连。
- APNs 使用“代理优先，代理不可用后直连”的独立策略。
- 境外、未知、普通加密 DNS、STUN 和不受支持的 UDP 不因故障回落直连。
- 运行时远程规则只能选择代理或拒绝策略，不能扩大直连集合。
- 远程规则 URL 固定到 40 位完整提交。
- 运行时不加载节点订阅，不执行第三方脚本，不启用 MITM 或 URL Rewrite。
- Wi-Fi/热点代理共享、HTTP API、外部控制器和 Web 面板均未开放。
- 配置保留轻量 HTTPS 健康/延迟检测，但不主动执行带宽测速，也不包含独立带宽测速策略组。
...
本项目对相关内容进行过格式筛选、拆分、去重或适配 Surge `RULE-SET` 的处理。继续公开分发或修改时，应核对并履行 AGPL-3.0 的许可证保留、修改说明、对应源代码提供及其他适用义务。
...
- 参考范围：Surge 配置、策略组、模块与规则的组织方式。

截至本次核对，仓库根目录未见统一许可证文件，README 另有其自身的使用限制和免责声明。本项目不复制、解释或替代那些条款；若要直接采用内容，应先确认权利和许可。
...
| APNs 目标 | 代理正常时为代理出口；回退后为真实出口 | 代理或用户真实出口 |
| 普通 DoH 服务 | DNS 查询、时间和代理连接元数据 | 代理出口 |
| APNs 专用 DoH 服务 | APNs/健康检测相关查询、时间和连接元数据 | 用户真实出口 |
...
| Apple 健康检测目标 | 连通性探测、时间和出口 IP | 基础网络或 APNs 直连路径 |
...
- APNs 专用 DoH 是明确直连例外，解析服务商和本地网络可以观察连接目标与时间，解析服务商还能看到查询。
...
配置保留 HTTPS 连通性和延迟检测。它们不会主动执行带宽测试，但仍会定期或按需向 Apple、Google 等检测目标建立连接，并暴露出口 IP、时间和基础请求元数据。上游服务规则中出现诊断域名并不表示配置会主动访问或运行相应测试。
...
- 代理节点、APNs、DNS、GitHub、jsDelivr 或健康检测目标始终可用。
...
- 核对代理节点提供者、服务条款和隐私政策。
- 遵守所在地法律、组织政策和网络服务条款。
- 评估国内、Apple 和 APNs 直连暴露真实 IP 的影响。
