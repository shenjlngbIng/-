...
本文件记录 Surge iOS Stable Fail-Closed R10.2 的直接来源、第三方内容、本地筛选和已知元数据缺口。R10.2 继续使用 R10 固定的上游快照与许可边界，并在本地修正规则顺序、广告误伤和服务覆盖。本文件不替代任何上游许可证，也不构成法律意见。

## 1. 本项目内容

以下部分属于本项目为当前仓库编写或重写的维护逻辑：

- R10.2 的策略闭环、规则顺序、DNS/UDP/IPv6/系统接管边界。
- APNs 精确规则先行，随后 Apple/Domestic 域名、服务、中国 GEOIP 和通用 UDP 守卫的组织方式。
...
R10.2 不在设备运行时下载规则。`Surge.conf` 将 26 个本地源文件固定内嵌：

- 22 个代理或拒绝源文件，原始 4583 条，按配置优先级与精确去重后内嵌 4429 条。
...
commit: 4d2f7d2f3acef6cc0a2efa18c44ff3ad49b92ada
```
...
R10.2 对自定义 `Ads_Custom_Extra.list` 进行可用性收窄：删除全部 IP-CIDR/IP-CIDR6 拦截，以及 HTTPDNS、B站 PCDN/Tracker、微信相关项和已确认产生业务误伤的宽泛关键词。该本地删改不改变其第三方内容的许可归属。

### 3.2 Apple 与国内直连源

| 文件 | 原始项 | R10 内嵌 | 本地修改 |
| --- | ---: | ---: | --- |
| `AppleCN.list` | 166 | 166 | 保留域名规则并启用扩展匹配；精确 APNs 规则更早固定代理 |
...
该次删除没有改变 R10.1 的 5599 条运行规则；R10.2 经广告与语义修正后当前为 5545 条。仓库只保留 `r10.lock.json` 固定的 26 个规则源文件，减少无效体积和来源元数据缺口。
