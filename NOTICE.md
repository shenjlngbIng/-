...
本项目在公开配置基础上进行了大幅重组与安全加固，包括策略闭环、规则顺序、DNS/DoH、IPv4/IPv6、UDP/QUIC/STUN、Apple/APNs 统一策略、控制面收缩、静态节点约束、远程规则固定提交、只读候选包验证及静态检查。
...
- APNs 不使用独立回退或专用 DNS，统一进入既有 `Apple` 策略。
...
| APNs 目标 | 与其他 Apple 目标一致，由 `Apple` 当前选择决定 | 代理或用户真实出口 |
...
- 核对代理节点提供者、服务条款和隐私政策。
- 遵守所在地法律、组织政策和网络服务条款。
- 评估国内与 Apple 直连暴露真实 IP 的影响；APNs 使用同一 Apple 选择。
